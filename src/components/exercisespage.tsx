    ...
    <div className="exercise-viewer">
      <div className="exercise-stage">
        <div className="exercise-content">
          ...
          {navButtonsVisible && (
            <button
              className="exercise-nav-inline exercise-nav-desktop exercise-nav-inline--next"
              onClick={nextEx}
...
function ExerciseQueueComponent({
  ...
  const paginationStatus =
    totalPages === 0 ? "" : `Page ${safePage} of ${totalPages}`;
...
export function ExercisesPage({
  ...
  const filteredExercises = React.useMemo(() => {
    const baseList = allExData.filter(
      (exercise): exercise is ExerciseData => exercise !== undefined,
    );
    const filtered = baseList.filter((exercise) => {
      ...
      if (meter !== "Anything" && String(exercise.meter) !== meter)
        return false;
  ...
  const currentExerciseIndex = React.useMemo(() => {
    ...
    return filteredExercises.findIndex(
      (exercise) => exercise.exIndex === selExercise.exIndex,
    );
  }, [filteredExercises, selExercise]);

  useEffect(() => {
    if (!selExercise) return;
    const stillExists = allExData.some(
      (exercise) => exercise?.exIndex === selExercise.exIndex,
    );
  ...
  const selectExerciseAtIndex = React.useCallback(
    (targetIndex: number) => {
  ...
  useEffect(() => {
    ...
    const targetIndex = filteredExercises.findIndex(
      (exercise) => exercise.exIndex === parsedIndex,
    );
    if (targetIndex !== -1) {
            ...
            <div className="ex-right">
              ...
              {!scoresRet ? (
                <div className="exercise-queue-loading">
                  ...
                  <span>
                    This process should take 2-10 seconds. If nothing changes
                    after 10 seconds, try sorting using the above criteria.
                  </span>
                </div>
              ) : null}
